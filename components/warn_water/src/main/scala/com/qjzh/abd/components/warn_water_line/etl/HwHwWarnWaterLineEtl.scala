package com.qjzh.abd.components.warn_water_line.etl

import com.qjzh.abd.components.comp_common.caseciew.SynAreaBean
import com.qjzh.abd.components.warn_water_line.service.HwHwWarnWaterLineServiceArea
...
//        (x.floor == report.floor) && (HwHwWarnWaterLineServiceArea.isContainInArea(report, x) != -1)
        (HwHwWarnWaterLineServiceArea.isContainInArea(report, x) != -1)
      }).take(1)
