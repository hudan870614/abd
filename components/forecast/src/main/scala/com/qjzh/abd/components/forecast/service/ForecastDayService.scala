...
object ForecastDayService {
  ...
  def DealForecast():Unit= {
    ...
    var curday:String = _forecastday
    if(curday.equals(""))
        curday = DateUtils.getCurTime("yyyyMMdd")
    Syslog.info("计算日期"+curday)
    //预测当天人流
    forecast(curday,0.2f,0.8f)
  }

  def forecast(forecastday:String,weight_cur:Float,weight_his:Float):Unit={
    ...
    forecastMemRdd.filter(x => x.hourmin.equals("2359")).distinct()
      .map( x => (x.stype,x)).groupByKey().collect().foreach(x => {
      val hourlist      = x._2.toList.sortBy(_.time).map(_.daynum)
      //箱型图剔除异常值
      ...
      if( fliterlist.length > 0 )
      {
        ...
        val forecastday_rowkey = forecastday+"_"+x._1
        ...
        if( x._1.equals("1_ALL") || x._1.equals("IMP_ALL") )
        {
